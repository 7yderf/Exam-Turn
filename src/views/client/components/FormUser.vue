  ...
  <Form_ v-slot="{ errors }" @submit="onSubmit" :validation-schema="schema" :initial-values="formValues"
    ...
    <div class="banners__inputs-content banners__inputs-content--column">
      ...
      <div class="banners__inputs">
        ...
        <div class="banners__inputs-radio banners__inputs-radio--checkbox" v-show="!formValues?.where.includes('home')">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      (val) => {
        if (val == "Video") {
          console.log("🚀 ~ file: FormUser.vue ~ line 108 ~ setup ~ val", val)
          console.log("🚀 ~ file: FormUser.vue ~ line 113 ~ setup ~ props.formValues?.url", props.formValues?.url)
    ...
    const onSubmit = (values) => {
      ...
      ApiService.post(`/api/banners/home/${id}`, formData, true)
        .then((res) => {
          console.log(res);
          console.log(values);
